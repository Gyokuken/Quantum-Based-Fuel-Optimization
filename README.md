...
> **Status.** The core pipeline (Phases 1-2) — fuel-rate prediction plus
> QUBO/annealing optimizers for cruise speed and weather routing, with an honest
> multi-solver benchmark — runs today. A fleet-level extension (multi-ship mission
> scheduling with a quantum-classical hybrid optimizer, including a run on real IBM
> quantum hardware) lives in [`fleet/`](fleet/) — see [`fleet/README.md`](fleet/README.md).
